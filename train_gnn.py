...
def evaluate(y_true, y_pred, label=""):
    rmse_ecsi = float(np.sqrt(mean_squared_error(y_true[:, 0], y_pred[:, 0])))
    rmse_sevi = float(np.sqrt(mean_squared_error(y_true[:, 1], y_pred[:, 1])))
    mae_ecsi  = float(mean_absolute_error(y_true[:, 0], y_pred[:, 0]))
    mae_sevi  = float(mean_absolute_error(y_true[:, 1], y_pred[:, 1]))
    r2_ecsi   = r2_score(y_true[:, 0], y_pred[:, 0])
    r2_sevi   = r2_score(y_true[:, 1], y_pred[:, 1])
    print(f"\n  [{label}]"
          f"  RMSE(ECSI)={rmse_ecsi:.4f}  RMSE(SEVI)={rmse_sevi:.4f}"
          f"  MAE(ECSI)={mae_ecsi:.4f}  MAE(SEVI)={mae_sevi:.4f}"
          f"  R2(ECSI)={r2_ecsi:.4f}  R2(SEVI)={r2_sevi:.4f}")
    return {
        "label"    : label,
        "RMSE_ECSI": rmse_ecsi, "RMSE_SEVI": rmse_sevi,
        "MAE_ECSI" : mae_ecsi,  "MAE_SEVI" : mae_sevi,
