...
Reviewer responses addressed
-----------------------------
R3.1.1  Edge weights passed as edge_attr to GATv2Conv (gnn_model.py)
R3.1.2  Edges from rook geometry, no ghost IDs, no diagonals (build_graph.py)
R3.1.3  Full metrics, baselines (OLS, MLP), graph ablation, hyperparams logged
R3.1.4  Transductive protocol — all nodes in forward pass, masks on loss only
R2.1    VIF reported in build_graph.py
R4.2.5  Baseline comparison saved to CSV and printed
"""
...
def evaluate(y_true, y_pred, label=""):
    ...
    print(f"\n  [{label}]  RMSE={rmse:.4f}  MAE={mae:.4f}  "
          f"R²(ECSI)={r2_ecsi:.4f}  R²(SEVI)={r2_sevi:.4f}")
...
if __name__ == "__main__":
    ...
    # 2. Splits (transductive: all nodes in forward pass) ─────────────────────
    ...
    # 9. Graph ablation (self-loops only) ─────────────────────────────────────
