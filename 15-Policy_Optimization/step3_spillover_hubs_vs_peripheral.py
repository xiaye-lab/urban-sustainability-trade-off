...
# ==============================================================================
# 1. CORE CALCULATION: CALCULATE BENEFIT
# ==============================================================================
...
def run_sensitivity_analysis():
    ...
    print(f"[2/5] Loading Trained Model...")
    ...
    print(f"[3/5] Identifying Hubs and Peripherals...")
    ...
    peri_candidates = [i for i in range(data.num_nodes) if deg[i] <= deg_median]

    hub_indices  = sorted(hub_candidates,  key=lambda i: inefficiency_score[i], reverse=True)[:NUM_SAMPLES]
    peri_indices = sorted(peri_candidates, key=lambda i: inefficiency_score[i], reverse=True)[:NUM_SAMPLES]

    print(f"   > Selected {len(hub_indices)} Hubs and {len(peri_indices)} Peripherals.")

    print(f"[4/5] Running Simulations...")
    plot_data = {
        'Strategy': [],
        'Hub_ECSI_Mult': [],  'Hub_SEVI_Mult': [],
        'Peri_ECSI_Mult': [], 'Peri_SEVI_Mult': []
    }
    ...
    for strat_name, config in STRATEGIES.items():
        ...
        def analyze_group_benefit(indices):
        ...
        h_loc, h_nei = analyze_group_benefit(hub_indices)
        p_loc, p_nei = analyze_group_benefit(peri_indices)

        h_ecsi_r = h_nei[0] / h_loc[0] if abs(h_loc[0]) > 1e-4 else 0
        h_sevi_r = h_nei[1] / h_loc[1] if abs(h_loc[1]) > 1e-4 else 0
        p_ecsi_r = p_nei[0] / p_loc[0] if abs(p_loc[0]) > 1e-4 else 0
        p_sevi_r = p_nei[1] / p_loc[1] if abs(p_loc[1]) > 1e-4 else 0

        plot_data['Strategy'].append(strat_name)
        plot_data['Hub_ECSI_Mult'].append(h_ecsi_r)
        plot_data['Hub_SEVI_Mult'].append(h_sevi_r)
        plot_data['Peri_ECSI_Mult'].append(p_ecsi_r)
        plot_data['Peri_SEVI_Mult'].append(p_sevi_r)
    ...
    rects2_e = ax1.bar(x + width/2, plot_data['Peri_ECSI_Mult'], width,
                       label='Inefficient Peripherals', color='#E2E2E2', alpha=0.9)
    ...
    max_ecsi = max(max(plot_data['Hub_ECSI_Mult']), max(plot_data['Peri_ECSI_Mult']))
    ...
    rects2_s = ax2.bar(x + width/2, plot_data['Peri_SEVI_Mult'], width,
                       label='Inefficient Peripherals', color='#E2E2E2', alpha=0.9)
    ...
    max_sevi = max(max(plot_data['Hub_SEVI_Mult']), max(plot_data['Peri_SEVI_Mult']))
    ...
    plt.tight_layout()
    plt.savefig("plot_spillover_hubs_vs_peripheral.png", dpi=300)
    print("Saved plot_spillover_hubs_vs_peripheral.png")
    plt.close()
