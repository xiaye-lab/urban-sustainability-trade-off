...
def plot_graph_structure(shp_file="fishnet_final_composite.shp",
                         color_by="ECSI", max_nodes=None, out_png=None):
    if out_png is None:
    ...
    labels = {
        "ECSI": "Environmental Composite Stress Index (ECSI)\n"
                "(Entropy-weighted: LST, PM25, CO2)",
        "SEVI": "Socio-Economic Vitality Index (SEVI)\n"
                "(Entropy-TOPSIS: NTL, POI, RND, POPU, PRICE_PSM)",
    }
    ax.set_title(labels.get(color_by, color_by), fontsize=13)
    ax.set_axis_off()
    plt.tight_layout()
